$(function () {
	var createKey = function (fn) {
		var email = $('#new_stud').find('section.email input').val();
		$('input[name="recipient_email"]').val(email);
	...
	$.each(teacherObj.class_info, function () {
		var that = this;
		if (that.school === eeDatas.currentSchool) {
			$('<option value="' + that.id + '">' + that.id + '</option>').appendTo('#class_assignment_classes');
			$('<option value="' + that.id + '">' + that.grade + ' - ' + that.name + '</option>').appendTo('.class_assigner');
		};
	});
	...
	var assignClass = function (fn) {
		...
		$('#class_assignment_classes').val($('#new_stud .class_assigner').val())
	...
	$('.code_box, .my_code').click(function () {
		if ($('.checkmark').hasClass('checked')) {
			...
			$('#member_form input[name="group_id"]').val("6");
			$(this).parent().parent().next().find('option').prop("selected", false);
		} else {
			$('.checkmark').addClass('checked');
			$('#member_form input[name="group_id"]').val("7");
			$(this).parent().parent().next().find('option').attr('selected','selected');
		}
	});
	...
	$('section.student a.header').click( function (e) {
        ...
        if ($(this).parent().find('section.edit_student').hasClass('open')) {
            ...
            $('.active_student').removeClass('active_student').find('span').text('View Profile');
        } else {
            $('section.content section.open').removeClass('open');
            $('.active_student').removeClass('active_student').find('span').text('View Profile');
            $(this).parent().find('section.edit_student').addClass('open');                    
            $(this).parent().find('section.option_buttons').addClass('open');
            $(this).find('.toggler').addClass('active_student').find('span').text('Close Profile');
        }
        return false;
    });
